...
## v5 tuner port — parity A/B attempts (worktree amd-dev-v5, branch gpu107-tuner-v5)

| utc_start | duration_s | jobid | nodelist | n_nodes | command | non-default env | output path | result |
|---|---|---|---|---|---|---|---|---|
| 2026-08-16T15:32:35Z | 163 | 14470 | amd-mi355x-5,6,7 | 3 | `run_parity.sh`: `validate_tuner_config.py --config gen_3n_t1.validated.csv --baseline-config (same) --baseline-plugin librccl-tunerv4-dn.so --plugin librccl-tunerv5-dn.so --repeats 7 --warmup-runs 8 --preflight 5 --min-bytes 4096 --max-bytes 536870912 --iters 20 --warmup 5 --plugin-must-fire --split-ranges` | sweep yaml env; INFO→file both arms | `results-tuning/2026-08-16-13-v5parity/parity_3n_attempt1_preflightfail/` | **preflight FAILED rc=3**: worst spread 50% at 1M (r2=12.68 vs ~18.9 median; single-run dips also at 4K/128K/256K/4M — nic-metrics-poller signature per INSIGHTS §7). A/B never started. |
| 2026-08-17T07:04:50Z | 164 | 14470 | amd-mi355x-5,6,7 | 3 | same as above (attempt 2) | same | `/opt/shared/ylerman/GPU-107/tuner-v5-ab-2026-08-16/parity_3n/` (copied later) | **preflight FAILED rc=3**: worst spread 73% at 32K; 12 of 18 sizes >10% spread and small/mid busbw HALVED vs yesterday (32K 0.35 vs 0.83, 1M ~8 vs 18.9) — fabric saturated by co-tenant `hai_test2` 2-node jobs on nodes [1-2] (ends ~12:02Z) and [4,9] (ends ~09:46Z). |
| 2026-08-17T07:12:51Z | 192 | 14470 | amd-mi355x-5 | 1 | `run_parity1n.sh`: 4 warmups + 7 interleaved reps/arm (v4 slot1, v5 slot2), `all_reduce_perf -b 4096 -e 536870912 -f 2 -g 1 -n 20 -w 5 -c 1 -A 1`, 8x1 | sweep yaml env; INFO→file; `NCCL_TUNER_CONFIG_FILE=gen_3n_t1.validated.csv` both arms | `results-tuning/2026-08-16-13-v5parity/parity_1n/` | selections identical 18/18; ≥256K within ±0.74%; 4K–128K v5 low 2.4–6.3% → order artifact (see next rows) |
| 2026-08-17T07:18:42Z | 150 | 14470 | amd-mi355x-5 | 1 | `run_ordertest.sh` part 1: same but v5 slot1, v4 slot2 | same | `results-tuning/2026-08-16-13-v5parity/revorder/` | gap FLIPS: v4 low 3.3–6.4% at 4K–32K → deficit follows slot, not plugin |
| 2026-08-17T07:21:12Z | 165 | 14470 | amd-mi355x-5 | 1 | `run_ordertest.sh` part 2: v4 in BOTH slots | same | `results-tuning/2026-08-16-13-v5parity/v4v4/` | pure slot effect with identical plugins: slot2 low up to 2.6% (16K). Order-balanced pool over parity_1n+revorder: 16/18 sizes within ±0.81%, psup 0.23–0.67 → **1n PARITY PASS** |
| 2026-08-17T07:24:47Z | 36 | 14470 | amd-mi355x-5 / 5,6,7 | 1+3 | `run_constprobe.sh`: v5 + empty rules, one run per {scale}x{noconst,extreme(144 absurd overrides)} | same + `NCCL_TUNER_CONSTANTS_FILE=constants_extreme.txt` in extreme arms | `results-tuning/2026-08-16-13-v5parity/constprobe/` | **constants DEAD on this runtime**: 0 selection diffs at 18 sizes, both scales, despite all-rank proof "applied 144 constants override(s)" + "Using DN-TUNER (v5)" |
| 2026-08-17T09:30:29Z | 336 | 14470 | amd-mi355x-5,6,7 | 3 | `run_parity.sh` forward pass (attempt 4 of the gated retry loop; same validator invocation as 08-16 attempt 1) | same | `results-tuning/2026-08-16-13-v5parity/parity_3n/` | **preflight PASS (10.7%) + A/B complete, rc=1 (no rules kept = parity)**. Selections identical 18/18; applied-config lines 50400 in BOTH arms; 16/18 sizes within ±0.73%; outliers 8K −4.6% (psup 0.27, v5-in-slot2, matches slot bias) and 16M +3.5% (psup 0.73, 9% spreads). Fabric context: hai_test2 [1-2] cross-leaf job GONE (replaced 08:20Z by single-node on 1); [4,9] leaf-local pair running — did not block. |
| 2026-08-17T09:36:10Z | 158 | 14470 | amd-mi355x-5,6,7 | 3 | `run_parity_swapped.sh` (arms swapped: baseline=v5, config=v4), first try | same | `results-tuning/2026-08-16-13-v5parity/parity_3n_swapped_s0_preflightfail/` (on /opt/shared) | preflight FAILED rc=3: 69% spread at 8K (transient dip). Gated 10-min retry loop `retry_swapped.sh` launched 09:39:15Z. |
| 2026-08-17T09:39–10:07Z | 4x ~160 | 14470 | amd-mi355x-5,6,7 | 3 | `retry_swapped.sh` attempts s1–s3 + instrumented s4i (2 Hz nicctl samplers on nodes 5/6/7) of `run_parity_swapped.sh` | same | `parity_3n_swapped_s{0..4i}_preflightfail/` (local copy in run dir) | all preflight FAIL rc=3: single-run dips at one size each (8K 69%, 8M 27%, 16K 65%, 2M 31%, 128K 58%). s4i correlation: pollers active during EVERY run (staggered ~10s bursts/30s x3 nodes) — presence alone doesn't discriminate; dipped run's 128K moment coincided with a node-5 nicctl burst. |
| 2026-08-17T10:15:45Z | 335 | 14470 | amd-mi355x-5,6,7 | 3 | `run_swapped_noexporter.sh`: USER-APPROVED stop of `amd-nic-metrics-exporter` on nodes 5,6,7 → `run_parity_swapped.sh` (arms swapped: baseline=v5, config=v4) → exporter restart (verified active 10:21:36Z, gap ~6 min) | same | `results-tuning/2026-08-16-13-v5parity/parity_3n_swapped/` + `EXPORTER_LOG` | **preflight PASS 8.7% (best 3n gate seen) + A/B complete rc=1.** First-try pass after 5 consecutive failures with exporter on → causal. Selections identical 18/18; worst gain +2.5% @32K psup 0.82. **Order-balanced 3n pool (14 runs/arm): 17/18 sizes within ±1.2%, 8K −4.5% = one 0.01 print-quantum psup 0.31, ZERO sizes large-and-significant → 3n PARITY PASS.** |
...
| 2026-08-17T09:06Z | ~90 (3 runs) | 14470 (peer alloc, agreed "WINDOW OPEN 2N", hard cutoff 10:00Z) | amd-mi355x-5,7 | 2 | 3× default `rccl_sweep.py --nodes 2` (no algo/proto/channels) — fabric gate before the 2n adaptive batch | sweep_config.yaml env; `MY_PATH=/opt/shared/ylerman/GPU-107/bin` | remote `/opt/shared/ylerman/GPU-107/optuna-live-2026-08-17/2n_gate/` (metrics copied into `results-tuning/2026-08-17-2-optuna2node/`) | **GATE PASS**: every size ≥1M within 2.5% of the 2026-08-16 2n default baseline (worst −2.42% @128M) despite hai_test2 on [4,9] — consistent with {5,7} being L1-leaf-local. |

| 2026-08-17T09:11Z | 1571 (102 runs) | 14470 (peer alloc, 2N window) | amd-mi355x-5,7 | 2 | `adaptive_search.py live --nodes 2 --grid 1,2,4,8,16,24,32,40,48 --anchors 8,24,48 --margin 15 --repeat-policy 3 --tol 0.5 --default-runs 0` (defaults covered by the gate runs) | as above | `results-tuning/2026-08-17-2-optuna2node/` | **102 runs vs the grid's 165 (-38%), 102/102 rc=0, 0 substituted, 34 configs; RING/LL pruned at anchors (first live combo prune).** vs 2026-08-16 2n winners (tol 0.5): 14/18 exact; all 4 diffs channel-only near-ties, live pick >= ref winner in same-session data (worst ref-side gap 1.21%). compare_vs_0816.json |

| 2026-08-17T10:26Z | ~110 (3 runs) | 14470 (peer alloc, "WINDOW OPEN 3N" 10:24Z) | amd-mi355x-5,6,7 | 3 | 3× default `rccl_sweep.py --nodes 3` — fabric gate before the 3n adaptive batch | sweep_config.yaml env; `MY_PATH=/opt/shared/ylerman/GPU-107/bin` | remote `/opt/shared/ylerman/GPU-107/optuna-live-2026-08-17/3n_gate/` (copied into `results-tuning/2026-08-17-3-optuna3node/gate/`) | **GATE PASS**: all sizes ≥1M within 4.8% of the 2026-08-16 3n default baseline, uniformly ~−2% (hai_test2 on [4,9] up; amd-nic-metrics-exporter active — median-of-3 damps its single-run dips). |

| 2026-08-17T10:31Z | 1843 (102 runs) | 14470 (peer alloc, 3N window) | amd-mi355x-5,6,7 | 3 | `adaptive_search.py live --nodes 3 --grid 1,2,4,8,16,24,32,40,48 --anchors 8,24,48 --margin 15 --repeat-policy 3 --tol 0.5 --default-runs 0` | as above | `results-tuning/2026-08-17-3-optuna3node/` | **102 runs vs the grid's 165 (-38%; 105 vs 165 incl. gate), 102/102 rc=0, 0 substituted, 34 configs; RING/LL pruned at anchors.** vs 2026-08-16 3n winners (tol 0.5): 15/18 exact; 3 diffs channel-only ties (worst in-session gap 0.47%, inside the 0.5% band). compare_vs_0816.json |

| 2026-08-17T11:36Z | 215 (15 runs) | 14583 (own alloc, -t 60) | amd-mi355x-8 (1n) and amd-mi355x-6,8 (2n; L2-leaf pair, NOT the published {5,7} — self-relative analysis only) | 1,2 | 15× default `rccl_sweep.py` (no algo/proto/channels): broadcast ×3 and reduce ×3 at 1n and 2n, all_reduce ×3 at 2n as control — input curves for Halperin hotspot detector | sweep_config.yaml env; `MY_PATH=/opt/shared/ylerman/GPU-107/bin`; DN_AJ on [5,7], hai_test2 on [4,9] and 1 (both leaf-local elsewhere, {6,8} shares L2 leaves with [4,9] — noted); amd-nic-metrics-exporter active (median-of-3 damps its single-run dips) | `results-tuning/2026-08-17-4-hotspot-mn/` (remote `/opt/shared/ylerman/GPU-107/optuna-live-2026-08-17/hotspot_mn/`) | **The 512K dip is 1-node-only.** Detector at 10%: broadcast 1n -21.9% and reduce 1n -20.2% @512K (stable in all 3 repeats each - not exporter noise); 2n curves monotonic, 0 hotspots even at 5%. all_reduce 2n control on {6,8} within ~2% of the {5,7} 08-16 baseline (worst -5.1% @2M from one dipped repeat). 15/15 rc=0. hotspots_th10.csv, hotspot_mn_curves.csv |

| 2026-08-18T13:02Z | 1290 (126 runs) | 14617 (own alloc, node 6, released 13:24Z) | amd-mi355x-6 | 1 | broadcast+reduce 1n grids: RING x {SIMPLE,LL} x ch{1,2,4,8,16,24,32,40,48,56} x 3 reps + 3 defaults each = 126 runs; part of the newcolls effort (1n portion pulled forward, node 6 free; 2n/3n stay on queued job 14606 {1,3,4}) | sweep_config.yaml env; `MY_PATH=/opt/shared/ylerman/GPU-107/bin` | `results-tuning/2026-08-18-1-newcolls/` (remote `/opt/shared/ylerman/GPU-107/optuna-live-2026-08-18/newcolls/`) | 126/126 rc=0. Replay vs fresh grids: conservative policy 18/18 exact both collectives, saving 0% (broadcast) / 15% (reduce) — 2-combo 20-config grids leave little to prune; 1+2 policy 25-45% saving at 15-18/18, worst gap 2.7%. replay_*_1n.json |

| 2026-08-18T17:07Z | 6900 total incl. node-4 stall (336 runs: 2n {1,3} 168, 3n {1,3,6} 168) | 14606 (own alloc {1,3,4}, -t 300, granted 17:06Z) | amd-mi355x-1,3 (2n) and 1,3,4 (3n) | 2,3 | driver_newcolls.sh: broadcast+reduce grids, RING x {SIMPLE,LL,LL128} x ch{1..48} x 3 reps + defaults = 336 runs | sweep_config.yaml env; `MY_PATH=/opt/shared/ylerman/GPU-107/bin`; mpirun-over-ssh from node 1; co-tenants at launch: jobs on 5,7 / 8 / 2 | `results-tuning/2026-08-18-1-newcolls/` | 336/336 rc=0 after node-4 swap. Replay: anchors {8,24,48} FAIL bcast/reduce mn (bimodal channel curve, ch=1 wins 32K-512K by 15-53%, worst miss 52.7%); anchors {1,8,24,48} recover 17-18/18 exact on all 9 grids, worst gap 1.45%, savings 4-26%. Detector: broadcast 3n dips -20.5% @64K. SUMMARY.md, anchor_fix_test.py |

| 2026-08-18T18:10Z | (amendment to 17:07Z row) | 14606 + 14814 (node 6 added) | 3n moved to amd-mi355x-1,3,6 | 2,3 | node 4 UNUSABLE: every 3n test on {1,3,4} hung to the 600s timeout (broadcast_3n_grid_rep1.log: 3x "Timeout after 600.1s"), and a 2n {1,4} probe also hung (probe_14/) while 2n {1,3} runs clean at ~10s/run — isolates the fault to node 4's fabric path. Driver killed 18:05Z, orphans cleaned, remainder relaunched as driver_newcolls2.sh with 3n on {1,3,6} (job 14814 holds node 6) | as 17:07Z row | same | node 4 needs cluster-admin attention; flagged |
